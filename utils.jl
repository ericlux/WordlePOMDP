...
path = "/Users/ericlux/Desk/Research/Mykel/wordlesolver"
...
SOLUTION_WORDS = parse_word_list(path*"/solutions_nyt.txt")

# list of words that are valid guesses, but will never be solutions
NONSOLUTION_WORDS = parse_word_list(path*"/nonsolutions_nyt.txt")
...
SOLUTION_WORD_TO_IDX = Dict((word,i) for (i,word) in enumerate(SOLUTION_WORDS))
ALL_WORD_TO_IDX = Dict((word,i) for (i,word) in enumerate(ALL_WORDS))
